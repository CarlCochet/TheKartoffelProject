...
namespace Stump.Server.WorldServer.Database.Idols
{
    ...
    public sealed class IdolTemplate : IAssignedByD2O, IAutoGeneratedRecord, ISaveIntercepter
    {
        ...
        public int? IdolSpellId
        {
            get
            {
                string spellName = IdolItem.Name.Split(' ')[0];
                SpellTemplate spell = SpellManager.Instance.GetSpellTemplates().FirstOrDefault(x => x.Name == spellName);
                if(spell != null)
                {
                    return spell.Id;
                }
        ...
        public byte IdolSpellLevel
        {
            get
            {
                string[] name = IdolItem.Name.Split(' ');
