...
namespace Stump.Server.WorldServer.Database.Achievements
{
    ...
    public class AchievementTemplate : IAutoGeneratedRecord, ISaveIntercepter, IAssignedByD2O
    {
        ...
        {
            var item = (DofusProtocol.Classes.Achievement)d2oObject;
            Id = (uint)item.id;
            NameId = item.nameId;
            ...
            Level = item.level;
            Order = item.order;
            KamasRatio = item.kamasRatio;
            ExperienceRatio = item.experienceRatio;
            KamasScaleWithPlayerLevel = item.kamasScaleWithPlayerLevel;
        ...
        {
            var ratio = KamasScaleWithPlayerLevel ? client.Character.Level : Level;
        ...
        public int GetExperienceReward(WorldClient client)
        {
            ...
            if (client.Character.Level > Level)
            {
                var rewardScale = (uint)Math.Min(client.Character.Level, Level * REWARD_SCALE_CAP);
            ...
            return (int)GetFixeExperienceReward(client.Character.Level, 1, ExperienceRatio) * loc6;
        }
