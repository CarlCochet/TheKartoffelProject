...
namespace Stump.Server.WorldServer.Game.Dialogs.Interactives
{
    public class ZaapDialog : IDialog
    {
        ...
        public ZaapDialog(Character character, InteractiveObject zaap, IEnumerable<Map> destinations)
        {
            ...
            foreach (var map in from map in character.SuperArea.Maps from interactive in map.GetInteractiveObjects()
                .Where(interactive => interactive.Template != null && interactive.Template.Type == InteractiveTypeEnum.TYPE_ZAAP) select map)
            {
                AddDestination(map);
            }
        }
