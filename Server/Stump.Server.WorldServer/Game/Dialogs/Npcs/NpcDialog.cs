...
namespace Stump.Server.WorldServer.Game.Dialogs.Npcs
{
    public class NpcDialog : IDialog
    {
        ...
        public virtual void ChangeMessage(NpcMessage message)
        {
            ...
            List<short> replies = new List<short>();
            foreach (var reply in message.Replies)
            {
                var result = true;
                if ((reply.CriteriaExpression != null && !reply.CriteriaExpression.Eval(Character) && reply.CanShow(Npc, Character)) || replies.Contains((short)reply.ReplyId))
                    result = false;
                if (result)
                    replies.Add((short)reply.ReplyId);
            }
