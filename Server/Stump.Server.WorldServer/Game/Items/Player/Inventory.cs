...
namespace Stump.Server.WorldServer.Game.Items.Player
{
    ...
    {
        [Variable(true)]
        const int MaxInventoryKamas = 2000000000;
        ...
        [Variable]
        public static readonly int TokenTemplateId = 12124;
        ...
        public void Save()
        {
            lock (Locker)
            {
                ...
                //if ((Tokens == null && Owner.Account.Tokens > 0u) || 
                //TODO Check actually it can be a security fail a bad desync can return points :p or lose too
                //    (Tokens != null && Owner.Account.Tokens != Tokens.Stack))
                //{
                //    Owner.Account.Tokens = (int) (Tokens?.Stack ?? 0);
                //    IpcAccessor.Instance.ProxyObject.UpdateAccount(Owner.Account);
                //}
            }
        }

        public BasePlayerItem Tokens { get; private set; }
        ...
        {
            //TokenTemplateId
            Tokens = ItemManager.Instance.CreatePlayerItem(Owner, TokenTemplateId, amount);
