...
namespace Stump.Server.WorldServer.Game.Items.BidHouse
{
    public class BidHouseItem : PersistantItem<BidHouseItemRecord>
    {
        ...
        {
            return new ObjectItem(63, (short)Template.Id, Effects.Select(x => x.GetObjectEffect()), Guid,
                (int)Stack);
        }

        public ObjectItemToSellInBid GetObjectItemToSellInBid()
        {
            return new ObjectItemToSellInBid((short)Template.Id, Effects.Select(x => x.GetObjectEffect()), Guid,
