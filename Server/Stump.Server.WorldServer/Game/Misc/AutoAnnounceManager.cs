...
namespace Stump.Server.WorldServer.Game.Misc
{
    public class AutoAnnounceManager : DataManager<AutoAnnounceManager>
    {
        ...
        public override void Initialize()
        {
            ...
            WorldServer.Instance.IOTaskPool.CallPeriodically(AnnouncesDelaySeconds * 1000, CheckVotes);
        }
        ...
        {
            foreach (var character in World.Instance.GetCharacters())
            {
                if (!character.Account.LastVote.HasValue || character.Account.LastVote.Value.AddHours(3) < DateTime.Now)
                {
                    character.DisplayNotification("Vous n'avez pas voté depuis plus de 3 heures !", NotificationEnum.ERREUR);
                }
            }
        }
