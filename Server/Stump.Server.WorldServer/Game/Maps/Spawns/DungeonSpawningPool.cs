            ...
            lock (m_locker)
            {
                ...
                foreach (var entity in spawn.GroupMonsters)
                {
                    if (entity.MinPartyMembers != null)
                        group.AddMonster(new Monster(entity.PossibleMonsterGrades.Shuffle().ToArray()[0], group), entity.MinPartyMembers.Value);
                    else
                        group.AddMonster(new Monster(entity.PossibleMonsterGrades.Shuffle().ToArray()[0], group));
                }
