    ...
    public abstract class FightActor : ContextActor, IStatsOwner
    {
        ...
        protected virtual void OnSpellCasted(SpellCastHandler castHandler)
        {
            if (castHandler.SpellLevel.Effects.All(effect => effect.EffectId != EffectsEnum.Effect_Invisibility) && VisibleState == VisibleStateEnum.INVISIBLE)
            {
                ...
                else if(!DofusEffects.Contains(castHandler.Spell.Template.Id))
                {
        ...
        List<int> DofusEffects = new List<int>
        {
         6149 , //dokoko
         8394 , //ochre
         8396 , //vulbis
        ...
        public SpellCastInformations TeleportCast(SpellCastInformations cast)
        {
            var portal = Fight.GetTriggers().FirstOrDefault(x => x is Portal && x.ContainsCell(cast.TargetedCell)) as Portal;
            if (portal != null)
            {
                if (Fight.GetActivableGamePortalsCount(portal.Caster.Team) < 2) return cast;

                if (portal.IsActive)
                {
                    var mpWithPortals = Fight.GetMarksMapPoint(portal.Caster.Team);
                    var links = Fight.GetLinks(new MapPoint(portal.CenterCell), mpWithPortals);
                    if (links.Count > 0)
                    {
                        try
                        {
                            ...
                            var distX = this.Position.Point.DistanceToCellX(new MapPoint(cast.TargetedCell));
                            var distY = this.Position.Point.DistanceToCellY(new MapPoint(cast.TargetedCell));
                            var dirx = this.Position.Point.OrientationToX(new MapPoint(cast.TargetedCell));
                            var diry = this.Position.Point.OrientationToY(new MapPoint(cast.TargetedCell));
                            ...
                            cast.Efficiency = cast.Efficiency + (new MapPoint(portal.CenterCell).ManhattanDistanceTo(new MapPoint(finalPortalCell)) /100);
                            cast.TargetedCell = this.Map.Cells[point2.CellId];
                            cast.CastCell = this.Map.Cells[finalPortal];
                        }
                        finally
                        {
        ...
        public virtual bool CastSpell(SpellCastInformations cast)
        {
            ...
            using (Fight.StartSequence(SequenceTypeEnum.SEQUENCE_SPELL))
            {
                ...
                if (fighter == null)
                {
                    ...
                    //    cast = TeleportCast(cast);
                    //}
                    var portal = Fight.GetTriggers().FirstOrDefault(x => x is Portal && x.ContainsCell(cast.TargetedCell)) as Portal;
                    if (portal != null && cast.Spell.Id != 5381)
                    {
                        if (portal.IsActive && Fight.GetActivePortalsCount(portal.Caster.Team) >= 2)
                        {
                            var mpWithPortals = Fight.GetMarksMapPoint(portal.Caster.Team);
                            var links = Fight.GetLinks(new MapPoint(portal.CenterCell), mpWithPortals);
                            if (links.Count > 0)
                            {
                                var firstPoint = new MapPoint(portal.CenterCell);
                                var character = Position.Point;

                                var finalPortal = links[links.Count - 1];

                                var portalOut = new MapPoint(finalPortal);

                                var x = character.X - firstPoint.X - portalOut.X;
                                var y = character.Y - firstPoint.Y - portalOut.Y;

                                var cellId = new MapPoint(x, y).CellId;

                                if (cellId < 0)
                                {
                                    cellId = (short)(cellId * -1);
                                }
                                cast.TargetedCell = Fight.Map.Cells[cellId];
                                ...
                                if (links.Count > 1)
                                {
                                    ...
                                    _bonusCoeffPortals = 1.0;
                                    var portals = new List<Portal>();
                                    var i = 0;
                                    while (i < links.Count)
                                    {
                                        portals.Add(Fight.GetTriggers(Fight.Map.Cells[links[i]]).FirstOrDefault() as Portal);
                                        i++;
                                    }
                                    i = 0;
                                    while (i < links.Count)
                                    {
                                        ...
                                        bonus = Math.Max(bonus, portal2.CastedSpell.CurrentSpellLevel.Effects[0].Value);
                                        if (previousPortal != null)
                                            dist = dist +
                                                   new MapPoint(portal2.CenterCell).DistanceTo(
                                                       new MapPoint(previousPortal.CenterCell));
                                        previousPortal = portal2;
                                        i++;
                                    }
                                    _bonusCoeffPortals = 2 + (bonus + 3 * dist) / 100.0;
                                }
                            }
                        }
                    }
        ...
        private readonly List<Portal> _unactivePortals = new List<Portal>();
        ...
        public virtual int InflictDamage(Damage damage)
        {
            ...
            if(damage.Source != null && damage.Source is MonsterFighter)
            {
                damage.Amount = (int)(damage.Amount * 1.7);
            }
            var permanentDamages = CalculateErosionDamage(damage.Amount);
            FractionBuff fractionBuff = GetBuffs(x => x is FractionBuff).FirstOrDefault() as FractionBuff;
            //Fraction
            if ( fractionBuff != null && !(damage is FractionDamage))
                return fractionBuff.DispatchDamages(damage);
            if (damage.Source != null && damage.Source._bonusCoeffPortals > 1.0)
            {
                //Console.WriteLine($"Damage : {damage.Amount} | BonusPortal : {damage.Source._bonusCoeffPortals} | Total : {damage.Amount * damage.Source._bonusCoeffPortals}");
                damage.Amount = (int)(damage.Amount * damage.Source._bonusCoeffPortals);
                damage.Source._bonusCoeffPortals = 0;
            }
        ...
        public bool IsInvisibleSpellCast(Spell spell)
        {
            ...
            return spellLevel.Effects.Any(entry => entry.EffectId == EffectsEnum.Effect_Trap) || // traps
                   ...
                   spell.Template.Id == 8394 || //ochre
                   spell.Template.Id == 8396 || //vulbis
                   spell.Template.Id == 8393 || //esmeralda
                   spell.Template.Id == 5952 || //turqueza
                   spell.Template.Id == 8418 || //zana
                   spell.Template.Id == 8395 || //crimson
                   spell.Template.Id == 6828 || //abysal
                   spell.Template.Id == 5453 || //watchers
                   spell.Template.Id == 5454; //cloudy
        }
