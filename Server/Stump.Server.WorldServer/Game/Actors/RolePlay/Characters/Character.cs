...
namespace Stump.Server.WorldServer.Game.Actors.RolePlay.Characters
{
    public sealed class Character : Humanoid, IStatsOwner, IInventoryOwner, ICommandsUser
    {
        ...
        void OnLoggedIn()
        {
			this.DisplayNotification("Bienvenue ! sur Geraxi pour les commande taper .help", NotificationEnum.INFORMATION);
			this.DisplayNotification("Les mise à jours se font régulièrement sur Geraxi", NotificationEnum.INFORMATION);
        ...
        {
            get { return (RoleEnum)Account.UserGroupId >= RoleEnum.Moderator ?
        ...
        public bool IncrementPrestige()
        {
            ...
            OpenPopup($"Tu es passé prestige {PrestigeRank} ! \r\nTu repasses donc niveau 1. \r\nTu dois te déconnecter et te reconnecter pour voir ton prestige et ton niveau !");
        ...
        public void CheckArenaDailyProperties()
        {
            ...
            var amountToken = (int)Math.Floor(ArenaDailyMaxRank / 7d);
            var amountKamas = (1000 * Level);
            var amountExperience = (100000 * Level);

            m_record.ArenaDailyDate = DateTime.Now;
            ArenaDailyMaxRank = 0;
            ArenaDailyMatchsCount = 0;
            ArenaDailyMatchsWon = 0;
            Inventory.AddItem(ItemManager.Instance.TryGetTemplate(ItemIdEnum.PIECE_DE_KAMA_GEANTE_12124), Math.Max(new CryptoRandom().Next(amountToken / 2, amountToken), 1));
            Inventory.AddItem(ArenaManager.Instance.TokenItemTemplate, amountToken);
            Inventory.AddKamas(amountKamas);
            AddExperience(amountExperience);

            DisplayNotification(NotificationEnum.KOLIZÉUM, amountKamas, amountToken, amountExperience);
        }
        ...
        {
            return (int)Math.Floor((2000 * (Level * (Level / 200d))));
        }

        public int ComputeWonExperience()
        {
            return (int)Math.Floor((20000 * (Level * (Level / 200d))));
        }
        ...
        public CompanionActor CreateCompanion(FightTeam team, CharacterFighter characterFighter)
        {
            ...
            if (Inventory.GetItems(CharacterInventoryPositionEnum.INVENTORY_POSITION_COMPANION).FirstOrDefault() == null)
                return null;

            CompanionRecord companion =
           Singleton<CompanionsManager>.Instance.GetCompanionById(
               Inventory.GetItems(CharacterInventoryPositionEnum.INVENTORY_POSITION_COMPANION).FirstOrDefault().Template.Id).FirstOrDefault();
            if (companion != null)
            {
        ...
        public override HumanInformations GetHumanInformations()
        {
            ...
            if (SelectedOrnament != null)
                options.Add(new HumanOptionOrnament((short)SelectedOrnament.Value));
