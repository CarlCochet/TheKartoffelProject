...
namespace Stump.Server.WorldServer.Game
{
    public class World : DataManager<World>
    {
        ...
        private Dictionary<int, MapScrollActionRecord> _mapScroll = new Dictionary<int, MapScrollActionRecord>();
        ...
        public void LoadSpaces()
        {
            ...
            logger.Info("Load maps scrolls...");
            _mapScroll = Database.Query<MapScrollActionRecord>(MapScrollActionRecordRelator.FetchQuery).ToDictionary(x => x.Id);
        ...
        public MapScrollActionRecord GetMapScroll(int mapId)
        {
        ...
        public Character GetCharacterByPattern(string pattern)
        {
            ...
            return WorldServer.Clients.Where<WorldClient>(entry => entry.Account.Login == name).
        ...
        public void Save()
        {
            lock (SaveLock)
            {
                ...
                {
                    IPCAccessor.Instance.SendRequest(new ServerUpdateMessage(WorldServer.Clients.Count, ServerStatusEnum.SAVING), delegate (Server.BaseServer.IPC.Messages.CommonOKMessage message)
                    {
                ...
                var clients = WorldServer.Clients;
                ...
                var document = new BsonDocument
                    {
                        ...
                        { "Players", WorldServer.Clients.Count },
                ...
                if (IPCAccessor.Instance.IsConnected)
                {
                    IPCAccessor.Instance.Send(new ServerUpdateMessage(WorldServer.Clients.Count, ServerStatusEnum.FINISH_SAVE));
                }
        ...
        public void SendAnnounceAllPlayersEvent() // Nom au hasard (Mettre un nom reconnaissable quand même...)
        {
            ...
            {
                character.DisplayNotification("<br>UN EVENTO VA A INICIAR</br> Prepárate para ganar!!!"); // Affichage temporaire (Notification)
