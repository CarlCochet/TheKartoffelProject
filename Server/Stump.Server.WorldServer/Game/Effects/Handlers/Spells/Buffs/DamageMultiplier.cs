...
namespace Stump.Server.WorldServer.Game.Effects.Handlers.Spells.Buffs
{
    ...
    public class DamageMultiplier : SpellEffectHandler
    {
        ...
        {
            if(Spell.Id == (int)SpellIdEnum.OAF)//BUTOR - OAF
            {
                ...
                if (damage.Spell != null && Caster.IsPoisonSpellCast(damage.Spell))
                    return;
                if(damage.Source.Position.Point.ManhattanDistanceTo(buff.Target.Position.Point) == 1)
                { 
                    damage.Amount = (int)Math.Round(damage.Amount * (Dice.DiceNum / 100.0));
                }
            }
            else if(Spell.Id == (int)SpellIdEnum.HORIZ)
            {
                var damage = token as Fights.Damage;
                if (damage == null)
                    return;
