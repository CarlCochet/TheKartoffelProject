...
namespace Stump.Server.WorldServer.Handlers.Guilds
{
	public class GuildHandler : WorldHandlerContainer
	{
		...
		public static void SendGuildInformationsMembersMessage(IPacketReceiver client, Guild guild)
		{
			try
			{
				client.Send(new GuildInformationsMembersMessage((from x in guild.Members
																 where CharacterManager.Instance.GetCharacterById(x.Record.CharacterId) != null
																 select x.GetNetworkGuildMember()).ToArray()));
			}
			catch { }
		}
