...
namespace Stump.Server.WorldServer.Commands.Commands
{
    public class InfoIP : InGameCommand
    {
        ...
        {
            trigger.Reply("Ip connectés: " + WorldServer.Clients.GroupBy(x => x.IP).Count());
        }
    }
    public class InfoCommand : SubCommandContainer
    {
        ...
        public override void Execute(TriggerBase trigger)
        {
            if (trigger.Args.HasNext)
                ...
                trigger.Reply("Uptime : " + trigger.Bold("{0}") + " Players : " + trigger.Bold("{1}"), WorldServer.Instance.UpTime.ToString(@"dd\.hh\:mm\:ss"), WorldServer.Clients.Count);
        }
    }
