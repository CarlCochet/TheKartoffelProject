...
namespace Stump.Server.AuthServer.Handlers.Connection
{
    public partial class ConnectionHandler : AuthHandlerContainer
    {
        ...
        public static void HandleClearIdentificationMessage(AuthClient client, ClearIdentificationMessage message)
        {
            ...
            //        string json = web.DownloadString("http://ip-api.com/json/" + client.IP);
            //        Console.WriteLine(json);
            //        json = json.Replace("{", "").Replace("}", "");
            //        string jsonExploded = json.Split(',').FirstOrDefault(x => x.Contains("\"city\""));

            //        web = new WebClient();
            //        json = web.DownloadString("http://tools.xioax.com/networking/v2/json/" + client.IP);
            //        json = json.Replace("{", "").Replace("}", "");
            //        string jsonVPNExploded = json.Split(',').FirstOrDefault(x => x.Contains("\"host-ip\""));
            //        Console.WriteLine(jsonVPNExploded);

            //        if (jsonExploded == null || jsonExploded.Split(':')[1].Contains("\"Roubaix\"") || jsonVPNExploded.Split(':')[1].Contains("true"))
            //        {
            //            SendIdentificationFailedMessage(client, IdentificationFailureReasonEnum.LOCKED);
            //            client.DisconnectLater(1000);
            //            return;
            //        }
